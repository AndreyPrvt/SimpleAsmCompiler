...
public class Processor {
    ...
    private Line parseIdentifier(String[] strings) {
        ...
        if (firstWord.length() > 9) {
            line.setError("Identifiers length more than 6 chars");
        }
        if (firstWord.endsWith(":")) {
            ...
            if (getKeyWords()[4].equals(next)) {
            ...
            } else if(getKeyWords()[21].equals(next)) {
                if (!MACROS) {
                    line.setError("Can\'t use this statement");
                }
                IN_MACROS = true;
                //procedure
                line.setError(setIdentifier(firstWord, getKeyWords()[21], ZERO));
                if (strings.length == 3 ){
                    line.setError(setIdentifier(strings[2], "MACRO_PARAM", ZERO));
                }

            } else if (getKeyWords()[22].equals(next)) {
                //end of procedure
                if (!MACROS) {
                    line.setError("Can\'t use this statement");
                }
                ...
                if (i != null) {
                    ...
                    line.setError("Macro not defined");
                }
                for (Identifier j : identifiers) {
                    if (j.getName().equals(firstWord)) {
                        if (j.getType() != "MACRO") {
                            line.setError("Unknown statement");
                        }
                    } else {
    ...
    private Line parseCommand(String[] strings) {
        ...
        if (rule == null) {
            ...
            if (Arrays.equals(rule, REG_MEM)) {
                ...
                if (firstPos >= 8 && firstPos <= 15 || secondPos >= 8 && secondPos <= 15) {
                    line.setOperandSizePrefix(OPERAND_SIZE_PREFIX);
                }
          ...
          //          line = generateMrmSib(data, ONLY_MEM);
                ...
                if (!isRegister(register) && (IN_MACROS == false)) { //ispr
                ...
                if (isIdentifier(strings[1])) {
                    ...
                    //line = generateMrmSib(data, rule);
                    line.setChangeSegmentPrefix(4);
                //    line.setChangeSegmentPrefix(getSegmentChangePrefix(data[0]));
                }
                line.setOperationCode(4);
                //line.setOperationCode(getOperationCodes().get(command) + memoryAddressMode);
    ...
    private String setIdentifier(String name, String type, BigInteger value) {
        ...
        if ((identifier == null) && (!type.equals("MACRO")) && (!type.equals("MACRO_PARAM"))) {
        ...
        if (second) {
            ...
            if ((i != null) && (!type.equals("MACRO_PARAM"))) {
                identifiers.add(new Identifier(name, "ERROR", offset, identifier.getName()));
                return "Redefined identifier";
            }
            if ((getKeyWords()[21].equals(type)) || (type.equals("MACRO_PARAM"))){
                identifiers.add(new Identifier(name, type, offset, "MACROS"));// ispravit tut
                return null;
            }
